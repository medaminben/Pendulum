...
QImage mat_to_qimage_ref(cv::Mat& mat, bool swap) {
    if(!mat.empty())
    switch(mat.type()) {
    case CV_8UC3 :{ if(!swap) return mat_to_qimage_ref(mat, QImage::Format_RGB888);
      return mat_to_qimage_ref(mat, QImage::Format_RGB888).rgbSwapped(); }
    case CV_8U : return mat_to_qimage_ref(mat, QImage::Format_Indexed8);
    case CV_8UC4 : return mat_to_qimage_ref(mat, QImage::Format_ARGB32);
    }
    return {};
}
